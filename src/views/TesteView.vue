<template>
  ...
  async function cadastrarVocabulo() {
    try {
      const { termo, significado, versao } = novoVocabulo.value;
      if (!termo || !significado || !versao) {
        erroCadastro.value = "Preencha todos os campos antes de cadastrar.";
        return;
      }
  
      await axios.post("vocabulo", { termo, significado, versao });
  ...
  async function consultarVocabulo() {
    try {
      ...
      mensagemConsulta.value = vocabulos.value.length === 0 && termoConsulta && versaoConsulta
        ? "Nenhum vocábulo foi encontrado para os critérios fornecidos."
        : '';
    } catch (ex) {
      erroCadastro.value = (ex as Error).message;
